...
const UserProfileSafetyEnhanced = ({
  ...
  const [profileEditInfo, setProfileEditInfo] = useState({
    editCount: 0,
    canEdit: true,
    lastEditDate: null as Date | null,
  });
  ...
  const fetchProfileEditInfo = async () => {
    try {
      ...
      const { data: profile } = await supabase
        .from("user_profiles")
        .select("profile_edit_count, can_edit_profile, last_profile_edit")
        .eq("id", authUser.id)
        .single();

      if (profile) {
        setProfileEditInfo({
          editCount: profile.profile_edit_count || 0,
          canEdit: profile.can_edit_profile !== false,
          lastEditDate: profile.last_profile_edit
            ? new Date(profile.last_profile_edit)
            : null,
        });
      }
    } catch (error) {
      console.error("Error fetching profile edit info:", error);
    }
  };

  const handleEditProfile = () => {
    if (!profileEditInfo.canEdit) {
      Alert.alert(
        "Edit Limit Reached",
        "You have reached the maximum number of profile edits (2). Please contact support if you need to make further changes.",
        [{ text: "OK" }]
      );
      return;
    }

    if (profileEditInfo.editCount >= 1) {
      Alert.alert(
        "Final Edit Warning",
        `This will be your ${
          profileEditInfo.editCount + 1
        }/2 profile edit. After this, you won't be able to edit your profile again. Are you sure you want to continue?`,
        [
          { text: "Cancel", style: "cancel" },
          { text: "Continue", onPress: () => setShowEditModal(true) },
        ]
      );
    } else {
  ...
  const saveProfileChanges = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("user_profiles").upsert(
        {
          ...
          email: user.email,
          branch_code: emailInfo.branchCode,
          joining_year: emailInfo.joiningYear,
      ...
      // Refresh edit info
      await fetchProfileEditInfo();

      Alert.alert(
        "Success",
        `Profile updated successfully! Edits remaining: ${
          2 - (profileEditInfo.editCount + 1)
        }`,
            ...
            >
              ...
              <TouchableOpacity
                style={[
                  styles.editButton,
                  {
                    backgroundColor: profileEditInfo.canEdit
                      ? isDarkMode
                        ? "#3B82F6"
                        : "#2563EB"
                      : isDarkMode
                      ? "#6B7280"
                      : "#9CA3AF",
                    opacity: profileEditInfo.canEdit ? 1 : 0.6,
                  },
                ]}
                onPress={handleEditProfile}
                disabled={!profileEditInfo.canEdit}
              >
                {profileEditInfo.canEdit ? (
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <ScrollView style={styles.modalContent}>
              <Text style={[styles.editWarning, { color: "#F59E0B" }]}>
                ⚠️ You can only edit your profile{" "}
                {2 - profileEditInfo.editCount} more time(s). Please ensure all
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(text) =>
                    setProfileData({ ...profileData, branch: text })
                  }
              ...
              <View style={styles.readOnlyInfo}>
                ...
                <Text
                  style={[
                    styles.readOnlySubtext,
                    { color: isDarkMode ? "#6B7280" : "#9CA3AF" },
                  ]}
                >
                  Parsed: {emailInfo.branchCode} • Joining Year:{" "}
                  {emailInfo.joiningYear}
                </Text>
              </View>
            </ScrollView>
